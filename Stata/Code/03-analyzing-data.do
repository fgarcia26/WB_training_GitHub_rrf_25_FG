...
*------------------------------------------------------------------------------- 
	...
	* Bar graph by treatment for all districts 
	gr bar 	area_acre_w, ///
			over(treatment) ///
			by(	district, row(1) note("") /// row 1 is to put all in the same row
				 legend(pos(6)) /// position (pos) is 6 on the clock
			...
			blabel(total, format(%9.1f)) /// this %9.1 is the decimal, check 9.2 to see the diff
	...
	* Distribution of non food consumption by female headed hhs with means
	...
	twoway	(kdensity nonfood_cons_usd_w if female_head==0, color(purple)) ///
			(kdensity nonfood_cons_usd_w if female_head==1, color(pink)) , ///
			xline(`mean_0', lcolor(purple) 	lpattern(dash)) ///
			xline(`mean_1', lcolor(pink) 	lpattern(dash)) ///
